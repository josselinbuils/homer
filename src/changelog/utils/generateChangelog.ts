...
export async function generateChangelog(
  ...
  if (commits.length > 0 && mergeRequestCommitsMaps.length === 0) {
    return commits
      .filter(filter)
      .map(({ message, title }) => {
        const jiraId = getJiraId(message);

        return `- ${title}${
          jiraId
            ? ` - [${jiraId}](https://manomano.atlassian.net/browse/${jiraId})`
            : ''
        }`;
      })
      .join('\n');
  ...
  return removeDuplicatedCommits(mergeRequestCommitsMaps)
    ...
    .map(({ mergeRequest, mergeRequestCommit }) => {
      const jiraId = getJiraId(mergeRequestCommit.message);
...
function getJiraId(commitMessage: string): string | undefined {
